...
var waypoints=[],containerEl=document.getElementById(canvasContId),parentContext=attrs.parentcontext||!1;debug=attrs.hasOwnProperty("debug")?attrs.debug:!1;var creds=attrs.usecredentials,windowEl=angular.element($window);windowEl.on("scroll",function(){scope.$apply(function(){scope.scroll=windowEl[0].scrollY})}),PDFJS.disableWorker=!1,scope.pageNum=pageToDisplay,scope.renderPage=function(num){renderTask[num]&&renderTask[num]._internalRenderTask.cancel(),pdfDoc.getPage(num).then(function(page){var viewport,pageWidthScale,renderContext;if(pageFit){viewport=page.getViewport(1);var clientRect=element[0].getBoundingClientRect();pageWidthScale=clientRect.width/viewport.width,scale=pageWidthScale}viewport=page.getViewport(scale);
// pageIDs has index starting at 0
var canvas=document.getElementById(scope.pageIDs[num-1]),ctx=canvas.getContext("2d");setCanvasDimensions(canvas,viewport.width,viewport.height);var handler=function(){scope.$apply(function(){scope.pageNum=num})};waypoints[num-1]=new Waypoint(parentContext?{element:canvas,handler:handler,offset:-1,// Default 0 is not good enough.
// It does not trigger the first page
context:containerEl}:{element:canvas,handler:handler,offset:-1}),Waypoint.refreshAll(),renderContext={canvasContext:ctx,viewport:viewport},renderTask=page.render(renderContext),renderTask.promise.then(function(){"function"==typeof scope.onPageRender&&scope.onPageRender()})["catch"](function(reason){console.log(reason)})})},/**
